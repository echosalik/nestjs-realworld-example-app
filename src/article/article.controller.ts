...
export class ArticleController {
  ...
  @ApiResponse({ status: 201, description: 'The article has been successfully Disliked.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  @Post(':slug/dislike')
  async dislike(@User('id') userId: number, @Param('slug') slug) {
    return this.articleService.dislike(userId, slug);
  }

  @ApiOperation({ summary: 'Undislike article' })
  @ApiResponse({ status: 201, description: 'The article has been successfully undisliked.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  @Delete(':slug/dislike')
  async unDislike(@User('id') userId: number, @Param('slug') slug) {
    return this.articleService.unDislike(userId, slug);
  }

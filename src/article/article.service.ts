...
export class ArticleService {
  ...
  async findAll(userId: number, query: any): Promise<IArticlesRO> {
    const user = userId ? await this.userRepository.findOne(userId, { populate: ['followers', 'favorites', 'dislikes'] }) : undefined;
    ...
    if ('tag' in query) {
      const regxp = query.tag.join("|");
      qb.andWhere({ tagList: new RegExp(`(^|,)(${regxp})(,|$)`) });
    }
  ...
  async dislike(id: number, slug: string): Promise<IArticleRO> {
    const article = await this.articleRepository.findOneOrFail({ slug }, { populate: ['author'] });
    const user = await this.userRepository.findOneOrFail(id, { populate: ['dislikes', 'followers'] });

    if (!user.dislikes.contains(article)) {
      user.dislikes.add(article);
      article.dislikesCount++;
    }
    await this.unFavorite(id, slug);
    await this.em.flush();
    return { article: article.toJSON(user) };
  }

  async unDislike(id: number, slug: string): Promise<IArticleRO> {
    const article = await this.articleRepository.findOneOrFail({ slug }, { populate: ['author'] });
    const user = await this.userRepository.findOneOrFail(id, { populate: ['followers', 'dislikes'] });

    if (user.dislikes.contains(article)) {
      user.dislikes.remove(article);
      article.dislikesCount--;
    }
